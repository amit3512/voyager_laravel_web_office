...
class NepaliCalender
{
	...
	// By Kshitiz
	public static function englishToNepali($j)
	{
	...
	public function getTodayDate($yy, $mm, $dd)
	{
		...
		if ($this->is_range_eng($yy, $mm, $dd) == false) {
			...
			$nepali_year = $this->englishToNepali($y);
			$nepali_month = $this->englishToNepali(sprintf('%02d', $m));
			$nepali_day  = $this->englishToNepali(sprintf('%02d', $total_nDays));
			...
			return $this->nep_date['nmonth'] . ' ' . $nepali_day  . ' ' . $nepali_year . ', ' . $this->nep_date['day'];
		}
	}
